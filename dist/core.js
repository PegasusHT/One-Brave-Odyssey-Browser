...
export const CAMPAIGN_STAGES=Array.from({length:3},(_,index)=>({id:index+1,label:`Stage ${index+1}`,firstBattle:index*BATTLES_PER_STAGE+1,lastBattle:(index+1)*BATTLES_PER_STAGE}));
...
export const ITEMS={weapon_t1:{id:'weapon_t1',name:'Wayfarer blade',slot:'weapon',tier:1,cost:0,bonus:0},weapon_t2:{id:'weapon_t2',name:'Cloudsteel saber',slot:'weapon',tier:2,cost:180,bonus:9},weapon_t3:{id:'weapon_t3',name:'Sunbreak edge',slot:'weapon',tier:3,cost:650,bonus:24},armor_t1:{id:'armor_t1',name:'Trail vest',slot:'armor',tier:1,cost:0,bonus:0},armor_t2:{id:'armor_t2',name:'Skyranger mail',slot:'armor',tier:2,cost:150,bonus:35},armor_t3:{id:'armor_t3',name:'Dawnwarden plate',slot:'armor',tier:3,cost:550,bonus:85},shield_t1:{id:'shield_t1',name:'No shield',slot:'shield',tier:1,cost:0,bonus:0},shield_t2:{id:'shield_t2',name:'Skyranger buckler',slot:'shield',tier:2,cost:120,bonus:0},shield_t3:{id:'shield_t3',name:'Dawnwarden shield',slot:'shield',tier:3,cost:450,bonus:0},weapon_t4:{id:'weapon_t4',name:'Set C blade',slot:'weapon',tier:4,cost:1400,bonus:45},weapon_t5:{id:'weapon_t5',name:'Set D blade',slot:'weapon',tier:5,cost:2800,bonus:72},weapon_t6:{id:'weapon_t6',name:'Set F blade',slot:'weapon',tier:6,cost:5200,bonus:105},armor_t4:{id:'armor_t4',name:'Set C armor',slot:'armor',tier:4,cost:1250,bonus:145},armor_t5:{id:'armor_t5',name:'Set D armor',slot:'armor',tier:5,cost:2500,bonus:220},armor_t6:{id:'armor_t6',name:'Set F armor',slot:'armor',tier:6,cost:4600,bonus:310},shield_t4:{id:'shield_t4',name:'Set C shield',slot:'shield',tier:4,cost:950,bonus:0},shield_t5:{id:'shield_t5',name:'Set D shield',slot:'shield',tier:5,cost:1900,bonus:0},shield_t6:{id:'shield_t6',name:'Set F shield',slot:'shield',tier:6,cost:3500,bonus:0}};
export const ARMORY_SETS=[['starter','Starter'],['setA','Set A'],['setB','Set B'],['setC','Set C'],['setD','Set D'],['setF','Set F']].map(([id,name],index)=>({id,name,tier:index+1,equipment:{weapon:`weapon_t${index+1}`,armor:`armor_t${index+1}`,shield:`shield_t${index+1}`}}));
const retiredEquipmentRefunds={helmet_t2:80,helmet_t3:280,bottoms_t2:100,bottoms_t3:350};
...
const enemyRoster=['Moss hornling','Twilight flutter','Pebble sentinel','Briar hornling','Violet skywing','Basalt keeper','Wildroot brute','Moonveil hunter','Stormstone giant','Elder thornhorn','Duskwing monarch','The Crown of Stone'];
export const ENEMIES=Array.from({length:TOTAL_BATTLES},(_,index)=>enemyRoster[index%enemyRoster.length]);
...
export function freshPlayer(now=Date.now()){return{version:2,name:'Rook',scarf:'#63dbc4',coins:120,xp:0,level:1,statPoints:0,skillPoints:1,stats:Object.fromEntries(STATS.map(k=>[k,5])),trainingLevels:Object.fromEntries(STATS.map(k=>[k,1])),equipment:{weapon:'weapon_t1',armor:'armor_t1',shield:'shield_t1'},inventory:['weapon_t1','armor_t1','shield_t1'],skills:[],stage:1,cleared:[],survivalUnlocked:false,potions:2,home:0,museum:0,lastCollection:now,totalHits:0,sessions:0,bestCombo:0,tutorials:{strength:false,dodge:false,block:false,crit:false},trainingMissions:{strength:1,dodge:1,block:1,crit:1},settings:{sound:false,motion:true}}}
export function normalizePlayer(raw,now=Date.now()){const p=freshPlayer(now);if(!raw||typeof raw!=='object')return p;p.name=typeof raw.name==='string'?raw.name.slice(0,18):p.name;p.scarf=['#63dbc4','#f0ad65','#b89be0'].includes(raw.scarf)?raw.scarf:p.scarf;for(const k of ['coins','xp','statPoints','skillPoints','potions','totalHits','sessions','bestCombo'])p[k]=whole(raw[k],p[k]);p.level=clamp(whole(raw.level,1),1,999);p.stage=clamp(whole(raw.stage,1),1,TOTAL_BATTLES+1);p.survivalUnlocked=raw.survivalUnlocked===true||p.stage>TOTAL_BATTLES||(whole(raw.version,1)<2&&p.stage>12);p.home=clamp(whole(raw.home),0,5);p.museum=clamp(whole(raw.museum),0,5);p.lastCollection=clamp(whole(raw.lastCollection,now,now),0,now);for(const k of STATS){p.stats[k]=clamp(whole(raw.stats?.[k],5),1,99999);p.trainingLevels[k]=clamp(whole(raw.trainingLevels?.[k],1),1,5)}const owned=[...new Set(Array.isArray(raw.inventory)?raw.inventory:[])];for(const id of owned)if(Object.hasOwn(retiredEquipmentRefunds,id))p.coins=whole(p.coins+retiredEquipmentRefunds[id],p.coins);p.inventory=[...new Set([...p.inventory,...owned.filter(k=>ITEMS[k])])];for(const slot of ['weapon','armor','shield'])p.equipment[slot]=p.inventory.filter(id=>ITEMS[id].slot===slot).reduce((best,id)=>ITEMS[id].tier>ITEMS[best].tier?id:best,p.equipment[slot]);p.skills=[...new Set((Array.isArray(raw.skills)?raw.skills:[]).filter(k=>SKILLS[k]))];p.cleared=[...new Set((Array.isArray(raw.cleared)?raw.cleared:[]).filter(k=>Number.isInteger(k)&&k>=1&&k<=TOTAL_BATTLES))];p.tutorials={strength:raw.tutorials?.strength===true,dodge:raw.tutorials?.dodge===true,block:raw.tutorials?.block===true,crit:raw.tutorials?.crit===true};p.trainingMissions={strength:Math.max(1,whole(raw.trainingMissions?.strength,1)),dodge:Math.max(1,whole(raw.trainingMissions?.dodge,1)),block:Math.max(1,whole(raw.trainingMissions?.block,1)),crit:Math.max(1,whole(raw.trainingMissions?.crit,1))};p.settings={sound:raw.settings?.sound===true,motion:raw.settings?.motion!==false};return p}
...
export function combatStats(p){return{hp:90+p.level*6+(ITEMS[p.equipment.armor]?.bonus||0),attack:Math.round(8+p.stats.strength*1.4+ITEMS[p.equipment.weapon].bonus),hit:clamp(.78+p.stats.accuracy*.006,.78,.98),dodge:clamp(p.stats.dodge*.008,0,.38),block:clamp(p.stats.block*.009,0,.5),crit:clamp(.05+p.stats.crit*.007,0,.45)}}
export function trainingCost(p,k){return 80*p.trainingLevels[k]**2}
export function buyItem(p,id){const item=ITEMS[id];if(!item)return{ok:false,message:'Unknown equipment.'};const current=p.inventory.filter(key=>ITEMS[key]?.slot===item.slot).reduce((best,key)=>ITEMS[key].tier>best.tier?ITEMS[key]:best,ITEMS[p.equipment[item.slot]]);if(item.tier<=current.tier)return{ok:false,message:'You already have this upgrade or better.'};if(item.tier!==current.tier+1)return{ok:false,message:'Get the next upgrade first.'};if(p.coins<item.cost)return{ok:false,message:'You need '+(item.cost-p.coins)+' more coins.'};p.coins-=item.cost;p.inventory.push(id);p.equipment[item.slot]=id;return{ok:true,message:item.name+' upgraded!'}}
