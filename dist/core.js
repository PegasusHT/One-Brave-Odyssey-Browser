...
export const GROUNDS=[{id:'strength',name:'Strength',icon:'⚔',color:'#ffba73',description:'Slash practice fruit at three heights. Kick the falling sparks.',benefit:'More attack damage'},{id:'accuracy',name:'Accuracy',icon:'◎',color:'#85ded7',description:'Throw when the moving sight crosses the target.',benefit:'More reliable hits'},{id:'dodge',name:'Dodge',icon:'➶',color:'#a7ccff',description:'Step into a safe lane before the thorns arrive.',benefit:'Evade enemy attacks'},{id:'block',name:'Block',icon:'⬡',color:'#ace2a0',description:'Raise your shield as the crystal reaches the ring.',benefit:'Reduce incoming damage'},{id:'crit',name:'Critical',icon:'✦',color:'#d8b0ed',description:'Tap as the pulse reaches the golden sweet spot.',benefit:'More critical strikes'}];
export const ITEMS={weapon_t1:{id:'weapon_t1',name:'Wayfarer blade',slot:'weapon',tier:1,cost:0,bonus:0},weapon_t2:{id:'weapon_t2',name:'Cloudsteel saber',slot:'weapon',tier:2,cost:180,bonus:9},weapon_t3:{id:'weapon_t3',name:'Sunbreak edge',slot:'weapon',tier:3,cost:650,bonus:24},armor_t1:{id:'armor_t1',name:'Trail vest',slot:'armor',tier:1,cost:0,bonus:0},armor_t2:{id:'armor_t2',name:'Skyranger mail',slot:'armor',tier:2,cost:150,bonus:35},armor_t3:{id:'armor_t3',name:'Dawnwarden plate',slot:'armor',tier:3,cost:550,bonus:85}};
...
export function freshPlayer(now=Date.now()){return{version:1,name:'Rook',scarf:'#63dbc4',coins:120,xp:0,level:1,statPoints:0,skillPoints:1,stats:Object.fromEntries(STATS.map(k=>[k,5])),trainingLevels:Object.fromEntries(STATS.map(k=>[k,1])),equipment:{weapon:'weapon_t1',armor:'armor_t1'},inventory:['weapon_t1','armor_t1'],skills:[],stage:1,cleared:[],potions:2,home:0,museum:0,lastCollection:now,totalHits:0,sessions:0,bestCombo:0,tutorials:{strength:false},settings:{sound:false,motion:true}}}
export function normalizePlayer(raw,now=Date.now()){const p=freshPlayer(now);if(!raw||typeof raw!=='object')return p;p.name=typeof raw.name==='string'?raw.name.slice(0,18):p.name;p.scarf=['#63dbc4','#f0ad65','#b89be0'].includes(raw.scarf)?raw.scarf:p.scarf;for(const k of ['coins','xp','statPoints','skillPoints','potions','totalHits','sessions','bestCombo'])p[k]=whole(raw[k],p[k]);p.level=clamp(whole(raw.level,1),1,999);p.stage=clamp(whole(raw.stage,1),1,13);p.home=clamp(whole(raw.home),0,5);p.museum=clamp(whole(raw.museum),0,5);p.lastCollection=clamp(whole(raw.lastCollection,now,now),0,now);for(const k of STATS){p.stats[k]=clamp(whole(raw.stats?.[k],5),1,99999);p.trainingLevels[k]=clamp(whole(raw.trainingLevels?.[k],1),1,5)}p.inventory=[...new Set([...p.inventory,...(Array.isArray(raw.inventory)?raw.inventory:[]).filter(k=>ITEMS[k])])];for(const slot of ['weapon','armor']){const id=raw.equipment?.[slot];if(ITEMS[id]?.slot===slot&&p.inventory.includes(id))p.equipment[slot]=id}p.skills=[...new Set((Array.isArray(raw.skills)?raw.skills:[]).filter(k=>SKILLS[k]))];p.cleared=[...new Set((Array.isArray(raw.cleared)?raw.cleared:[]).filter(k=>Number.isInteger(k)&&k>=1&&k<=12))];p.tutorials={strength:raw.tutorials?.strength===true};p.settings={sound:raw.settings?.sound===true,motion:raw.settings?.motion!==false};return p}
...
export function combatStats(p){return{hp:90+p.level*6+ITEMS[p.equipment.armor].bonus,attack:Math.round(8+p.stats.strength*1.4+ITEMS[p.equipment.weapon].bonus),hit:clamp(.78+p.stats.accuracy*.006,.78,.98),dodge:clamp(p.stats.dodge*.008,0,.38),block:clamp(p.stats.block*.009,0,.5),crit:clamp(.05+p.stats.crit*.007,0,.45)}}
...
const multiplier=1+.25*(p.trainingLevels[session.kind]-1);
const bonusStrength=strength?whole(session.bonusStrength):0;
const bonusXp=strength?whole(session.bonusXp):0;
const goals=strength?whole(session.goals):0;
const gain=Math.floor(session.hits*multiplier/4)+bonusStrength;
const xp=session.hits*3+bonusXp;
const levels=addXp(p,xp);
p.stats[session.kind]+=gain;
p.totalHits+=session.hits;
p.bestCombo=Math.max(p.bestCombo,session.bestCombo);
const complete=!strength&&session.elapsed>=29.9;
const skill=complete&&session.hits>=8?1:0;
if(strength?session.elapsed>0||session.hits>0:complete)p.sessions++;
p.skillPoints+=skill;
return{gain,xp,levels,skill,hits:session.hits,bestCombo:session.bestCombo,goals,bonusStrength,bonusXp}
}
