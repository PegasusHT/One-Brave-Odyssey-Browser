...
export function hero(c,x,y,s=1,equipment={},time=0,attack=0,hit=0,scarf='#63dbc4',action=null){const a=EQUIPMENT_ART[equipment.armor]||EQUIPMENT_ART.armor_t1;const w=EQUIPMENT_ART[equipment.weapon]||EQUIPMENT_ART.weapon_t1;c.save();c.translate(x,y);c.scale(s,s);ellipse(c,0,0,45,10,'#183c4844');c.translate(attack*18,Math.sin(time*3)*2);if(action==='kick'&&attack>0){c.save();c.translate(-14,-36);c.rotate(attack*1.4);rect(c,-10,0,18,48,6,'#293e4d');rect(c,-17,38,27,12,5,'#524e42');c.restore()}else rect(c,-24,-38,18,33,6,'#293e4d');rect(c,8,-38,18,33,6,'#293e4d');rect(c,-29,-13,27,12,5,'#524e42');rect(c,7,-13,29,12,5,'#524e42');poly(c,[[-23,-94],[24,-94],[30,-39],[-29,-39]],a.base,'#274c58');rect(c,-30,-85,20,24,8,a.trim);rect(c,13,-85,20,24,8,a.trim);rect(c,-26,-48,54,9,3,'#625a42');rect(c,-4,-49,12,12,2,'#e4bd6a');poly(c,[[-24,-96],[-61,-82],[-45,-69],[-67,-55],[-19,-66]],scarf);ellipse(c,-3,-117,29,33,'#bd7e55');ellipse(c,1,-120,27,31,'#f0bd83');poly(c,[[-29,-120],[-33,-143],[-17,-153],[-4,-151],[10,-158],[20,-145],[30,-139],[27,-123],[15,-136],[3,-130],[-9,-138],[-15,-115]],'#3d4350');rect(c,0,-123,5,8,2,'#203443');rect(c,18,-123,5,8,2,'#203443');c.strokeStyle='#ba7d55';c.lineWidth=2;c.beginPath();c.arc(13,-110,5,0,Math.PI);c.stroke();rect(c,-25,-99,52,13,6,scarf);c.save();c.translate(28,-76);c.rotate(-.5+attack*(action==='0'?.65:action==='1'?1.65:action==='2'?2.6:action==='kick'?.1:1.9));rect(c,3,-10,12,34,6,'#edb480');rect(c,8,-51,8,50,3,'#725541');rect(c,-4,-38,32,7,2,'#dfb464');poly(c,[[8,-38],[5,-w.length-25],[12,-w.length-46],[21,-w.length-25],[17,-38]],w.blade,'#345867');poly(c,[[12,-w.length-46],[12,-40],[17,-40],[21,-w.length-25]],w.edge);c.restore();if(hit>0){c.globalAlpha=hit*.5;ellipse(c,0,-85,45,70,'#fff7df')}c.restore()}
...
const handX=1140-30*reach+35*windup*(1-reach)+12*low,handY=373-33*reach-38*windup*(1-reach)+95*low;
...
if(release===0){ellipse(c,handX-4,handY-11,19,18,'#f3932d');ellipse(c,handX-9,handY-17,7,6,'#ffd278')}
c.restore();
poly(c,[[1250,462],[1320,462],[1308,509],[1260,509]],'#bd8f5e','#886447');
