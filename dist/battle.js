...
export class Battle{
constructor(player,stage,feedback,survival=false){this.player=player;this.stage=stage;this.survival=survival;this.wavesCleared=0;this.wave=0;this.stats=combatStats(player,stage);this.hp=this.stats.hp;this.foe=enemyStats(stage,0);this.enemyHp=this.foe.hp;this.time=0;this.nextTurn=1;this.turn=0;this.attacks=0;this.done=false;this.won=false;this.rewarded=false;this.event=0;this.heroAttack=0;this.normalAttackAt=-Infinity;this.enemyAttack=0;this.heroHit=0;this.enemyHit=0;this.ward=0;this.stunTurns=0;this.poison=null;this.tonicUses=0;this.cooldowns=Object.fromEntries([...Object.keys(SKILLS),'tonic'].map(key=>[key,0]));this.effects=new Pool(30);this.feedback=feedback;this.message='Attacks are automatic. Watch for a golden opening.'}
step(dt){
if(this.done)return;
this.time+=dt;this.heroAttack=Math.max(0,this.heroAttack-dt*3);this.enemyAttack=Math.max(0,this.enemyAttack-dt*3);this.heroHit=Math.max(0,this.heroHit-dt*3);this.enemyHit=Math.max(0,this.enemyHit-dt*3);this.event=Math.max(0,this.event-dt);
...
if(this.hp<=0){this.done=true;this.won=false;this.event=0;this.poison=null;this.stunTurns=0}
}
}
...
if(this.wave>=2&&!this.survival){this.done=true;this.won=true;return}
...
if(action==='event'){if(this.event<=0)return false;this.event=0;this.heroAttack=1;this.damageEnemy(Math.round(this.stats.attack*1.25),'PERFECT');return true}
if(action==='tonic'){
...
c.restore();
const enemyX=980-this.enemyAttack*26;
if(this.poison)ellipse(c,enemyX,479,105,24,'#91d57855');
enemy(c,enemyX,495,1.8,this.foe.type,t,this.enemyHit);
if(this.poison||this.stunTurns){c.save();c.font='700 18px "DM Sans",sans-serif';c.textAlign='center';if(this.poison){c.fillStyle='#d3ffac';c.fillText('POISON',enemyX,305)}if(this.stunTurns){c.fillStyle='#ffe397';c.fillText('STUN',enemyX,280)}c.restore()}
for(const effect of this.effects.items){if(!effect.active)continue;c.globalAlpha=clamp(effect.life,0,1);c.font='700 28px "DM Sans",sans-serif';c.textAlign='center';c.strokeStyle='#224350';c.lineWidth=5;const y=effect.y-(effect.x===410?158*1.65*(HERO_SCENE_ART.arenaScale-1):0);c.strokeText(effect.text,effect.x,y);c.fillStyle=effect.color;c.fillText(effect.text,effect.x,y)}c.globalAlpha=1
}
}
